...
function initTeamPick(){
  ...
  const sel = $("teamPick");
  sel.innerHTML = `<option value="">All Teams</option>` + teams.map(t=>`<option value="${esc(t)}">${esc(teamDisp(t))}</option>`).join("");
...
function renderSquad(team){
  if(!team){
    return `<div class="card squadDark"><div class="muted">Team select karo to squad dikhega.</div></div>`;
  }

  // Prefer squadMeta (role/captain/wk) for rich UI
  const meta = getSquadMeta(team);
    ...
    <div class="card">
      ...
      <div class="list">
        ${list.map(p=>{
          ...
          const r = (p.role||"").toString().toUpperCase();
          const chips = [];
          if(p.isCaptain) chips.push(`<span class="chip c">C</span>`);
          if(p.isViceCaptain) chips.push(`<span class="chip vc">VC</span>`);
          if(p.isWicketKeeper) chips.push(`<span class="chip wk">WK</span>`);
          if(r) chips.push(`<span class="chip role ${r}">${esc(r)}</span>`);
          const sub = r ? `<span class="muted small">${esc(roleLabel(r))}</span>` : "";
          return `
            <div class="item squadItem">
              <div class="left">
